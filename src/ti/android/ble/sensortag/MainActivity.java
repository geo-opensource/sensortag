...
public class MainActivity extends ViewPagerActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mSectionsPagerAdapter.addSection(HelpView.newInstance("help_scan.html", R.layout.fragment_help, R.id.webpage), "Help");
    ...
    private boolean checkDeviceFilter(BluetoothDevice device) {
        int numRecognizedDevices = mDeviceFilter.length;
        if (numRecognizedDevices > 0) {
            if (device != null) {
                for (String recognizedDevice : mDeviceFilter) {
                    if (recognizedDevice.equals(device.getName())) {
                        return true;
                    }
                }
            }
            return false; // null arg device, or no devices found
        } else {
